...
**Corrected for v68.0.** `BindingObjectCustomExt` does not appear anywhere in the RLM Developer Guide's DRO Standard Objects inventory (printed pp. 1830–1831) and is annotated here as **not a real v68 standard object** — removed below. The `ProductFulfillmentScenario` → step-definition relationship also pointed at the wrong granularity (it links to the step-definition *group*, not an individual step definition).
